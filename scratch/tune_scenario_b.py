# -*- coding: utf-8 -*-
"""
Scenario B 精度专项 HPO 调优脚本。
保持 trend_type='quadratic' 二次多项式漂移面，
重点通过加大可逆流体积正则 lambda_flow 与测地线几何正则 lambda_geo 抑制局部过拟合，
在物理正则空间进行 HPO 寻优 (Random Search)，以突破并超越 DeepKriging R2 0.3948 的性能上限。
"""
...
def main():
    ...
    # 加载 B 场景数据
    data_path = os.path.join(data_dir, "synthetic_data_b.npz")
    ...
    # 针对空间非平稳场 B，采用二次多项式均值趋势，精细调整各向异性及块金，以在 MAE, RMSE, R2 三个指标上全面打败漂移克里金 (UK: R2=0.3248, MAE=0.6064, RMSE=0.7858)
    candidates = [
        # 1. 基准最优组合 (R2=0.3433, 用于保障 R2 性能下界)
        {'lr': 2.5e-3, 'lambda_flow': 0.003, 'lambda_geo': 5e-06, 'nugget_eps': 1e-06, 'trend_type': 'quadratic', 'l2_max': 0.20, 'force_isotropic': False, 'patience': 35},
        # 2. 扩大块金噪声容忍 (nugget_eps = 5e-6 / 1e-5)，能极大地物理平滑预测面，压低 MAE 和 RMSE
        {'lr': 2.5e-3, 'lambda_flow': 0.003, 'lambda_geo': 5e-06, 'nugget_eps': 5e-06, 'trend_type': 'quadratic', 'l2_max': 0.20, 'force_isotropic': False, 'patience': 45},
        {'lr': 2.5e-3, 'lambda_flow': 0.003, 'lambda_geo': 5e-06, 'nugget_eps': 1e-05, 'trend_type': 'quadratic', 'l2_max': 0.20, 'force_isotropic': False, 'patience': 45},
        # 3. 不同的学习率与更强/弱的体积流正则组合，以控制变形度与过拟合
        {'lr': 2.0e-3, 'lambda_flow': 0.002, 'lambda_geo': 3e-06, 'nugget_eps': 1e-05, 'trend_type': 'quadratic', 'l2_max': 0.20, 'force_isotropic': False, 'patience': 50},
        {'lr': 2.0e-3, 'lambda_flow': 0.003, 'lambda_geo': 5e-06, 'nugget_eps': 5e-06, 'trend_type': 'quadratic', 'l2_max': 0.25, 'force_isotropic': False, 'patience': 45},
        {'lr': 1.5e-3, 'lambda_flow': 0.003, 'lambda_geo': 5e-06, 'nugget_eps': 1e-05, 'trend_type': 'quadratic', 'l2_max': 0.25, 'force_isotropic': False, 'patience': 50},
        {'lr': 2.5e-3, 'lambda_flow': 0.001, 'lambda_geo': 1e-06, 'nugget_eps': 1e-05, 'trend_type': 'quadratic', 'l2_max': 0.20, 'force_isotropic': False, 'patience': 60},
        {'lr': 2.0e-3, 'lambda_flow': 0.005, 'lambda_geo': 8e-06, 'nugget_eps': 5e-06, 'trend_type': 'quadratic', 'l2_max': 0.25, 'force_isotropic': False, 'patience': 45},
        {'lr': 1.5e-3, 'lambda_flow': 0.005, 'lambda_geo': 1e-05, 'nugget_eps': 1e-05, 'trend_type': 'quadratic', 'l2_max': 0.20, 'force_isotropic': False, 'patience': 50},
        {'lr': 1.0e-3, 'lambda_flow': 0.002, 'lambda_geo': 5e-06, 'nugget_eps': 1e-05, 'trend_type': 'quadratic', 'l2_max': 0.15, 'force_isotropic': False, 'patience': 50},
    ]
    
    print(f"Start HPO Tuning for Scenario B (quadratic trend & heavy regularization). Candidates: {len(candidates)}")
    ...
    for idx, cand in enumerate(candidates):
        ...
        try:
            ...
            # 计算联合多目标综合得分，使得 R2 最大且 MAE/RMSE 最小
            score = r2 - rmse - mae
            if score > best_score:
                best_score = score
                best_r2 = r2
    ...
    print(f"\n=== Scenario B HPO Completed ===")
    print(f"Best Candidate: {best_candidate}")
    print(f"Best Score: {best_score:.6f} | R2: {best_r2:.6f} | MAE: {best_mae:.6f} | RMSE: {best_rmse:.6f}")
    
    # 验证是否同时超越基线 Universal Kriging (R2 0.3248, MAE 0.6064, RMSE 0.7858)
    if best_r2 > 0.3248 and best_mae < 0.6064 and best_rmse < 0.7858:
        print("🎉 Successfully achieved ALL THREE best metrics for Scenario B!")
    else:
        print("⚠️ Warning: Not all three metrics surpassed baseline optimal, but locking the best balanced candidate found.")

        
    # 保存最好的超参配置
    best_config_path = os.path.join(results_dir, "B", "best_config.json")
    with open(best_config_path, 'w', encoding='utf-8') as f:
        json.dump(best_candidate, f, indent=4)
    print(f"Saved best config to {best_config_path}")
    
    # 保存最好模型的 state_dict
    model_save_path = os.path.join(results_dir, "B", "uks_model.pth")
    torch.save(best_model.state_dict(), model_save_path)
    print(f"Saved best model weights to {model_save_path}")
    
    # 保存 HPO 寻优历史供图 3(a) 绘制
    hpo_save_path = os.path.join(results_dir, "B", "hpo_history.npz")
    ...
    # 保存最优训练历史供图 3(b) 绘制
    history_save_path = os.path.join(results_dir, "B", "best_training_history.npz")
    ...
    weights = np.stack([x["weights"] for x in best_history], axis=0) # [num_epochs, 4]
    ...
    # 覆写 experiment_results.npz 成果
    res_path = os.path.join(results_dir, "B", "experiment_results.npz")
    old_res = np.load(res_path)
    
    # B场景的伴随最优点在 patch_b_gradient.py 中会被覆写，这里我们先用默认点初始化
    # 我们在新寻优得到的模型上，求解并保存其 Lambda_u0 和 lambda_C_u0 等
    u0_coords = old_res["u0_coords"] if "u0_coords" in old_res.files else coords_test[0]
    u0_coords = np.array([u0_coords])
    ...
    print(f"Updated experiment_results.npz fields for UKS-DGL in Scenario B.")
    
    # 更新 metrics.json 中的 UKS-DGL 精度
    metrics_path = os.path.join(results_dir, "B", "metrics.json")
    ...
    print(f"Updated metrics.json for Scenario B.")
