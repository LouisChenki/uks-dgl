# -*- coding: utf-8 -*-
"""
Scenario F 精度专项 HPO 调优脚本。
针对偏态多中心非高斯随机场进行 HPO 寻优 (Random Search)，
重点搜索可逆流层数与较弱的体积正则，以超越普通克里金 OK R2 0.6763 的性能上限。
"""
...
def eval_candidate(candidate, coords_train, Z_train, coords_test, Z_test_raw, mean_Z, std_Z, device, dtype):
    ...
    # 实例化单场景专属 UKSModel，配置 4 层体积流以支撑偏态还原
    model = UKSModel(
        in_dim=2,
        flow_hidden_dim=flow_hidden,
        num_flow_layers=candidate.get('num_flow_layers', 4),
        embed_dim=16,
...
def main():
    ...
    # 加载 F 场景数据
    data_path = os.path.join(data_dir, "synthetic_data_f.npz")
    ...
    print(f"Start HPO Tuning for Scenario F. Total candidates: {len(candidates)}")
    ...
    # 保存最好的超参配置
    best_config_path = os.path.join(results_dir, "F", "best_config.json")
    with open(best_config_path, 'w', encoding='utf-8') as f:
        json.dump(best_candidate, f, indent=4)
    print(f"Saved best config to {best_config_path}")
    
    # 保存最好模型的 state_dict
    model_save_path = os.path.join(results_dir, "F", "uks_model.pth")
    torch.save(best_model.state_dict(), model_save_path)
    print(f"Saved best model weights to {model_save_path}")
    
    # 保存 HPO 寻优历史
    hpo_save_path = os.path.join(results_dir, "F", "hpo_history.npz")
    ...
    # 保存最优训练历史
    history_save_path = os.path.join(results_dir, "F", "best_training_history.npz")
    ...
    res_path = os.path.join(results_dir, "F", "experiment_results.npz")
    ...
    print(f"Updated experiment_results.npz fields for UKS-DGL in Scenario F.")
    
    # 更新 metrics.json 中的 UKS-DGL 精度
    metrics_path = os.path.join(results_dir, "F", "metrics.json")
    ...
    print(f"Updated metrics.json for Scenario F.")
